...
**Stav:** F1 v2.2.0 + F2 v2.0.2 → Pilot choux 2026-04-17 ✅ → Madeleines re-run 2026-04-25 ✅ (O1+O2+O3 fixes, 6/6 full reviews, GPT+Nemotron FIXED) → D60 formalizován → DALŠÍ: GitHub Issues pro 6 REST recenzí; F1 v2.2.1 patche (P1–P5) + F3 Auditor prompt
...
| D60 | **Orchestrator fixes O1+O2+O3 verified; madeleines re-run 6/6 complete (2026-04-25).** Three fixes applied to `scripts/run-review-panel.js`: O1 — `content_type: ""` → `"text/html"` via `contentTypeForFile()` helper (fixes `x-amz-meta-is_text_only: "false"` → `"true"` in S3 Step A response, enabling Nemotron to read attachment); O2 — `model_preference: "gpt54"` → `"gpt54_thinking"` (fixes GPT attachment support — `gpt54` and `gpt54_thinking` are distinct Perplexity backend entries, only `_thinking` variant supports S3 attachments); O3 — added `x-perplexity-request-endpoint`, `x-perplexity-request-reason: "perplexity-query-state-provider"`, `x-perplexity-request-try-number: "1"`, `x-request-id: <frontend_uuid>` headers to ask call (uniformizes with UI behavior confirmed via 4-capture cross-check). Smoke test on `example-v0_1_0` confirmed S3 upload + `is_text_only: true` working. Production re-run `run-20260425T205535Z` results: claude-sonnet-46 52342 chars ✅, gpt-54 68049 chars ✅ (was 1618 pre-fix → **FIXED**), gemini-31-pro-high 47275 chars ✅, grok 39973 chars ✅, nemotron-3-super 43437 chars ✅ (was 1587 pre-fix → **FIXED**), sonar-deep-research 35344 chars (still no attachment support — multi-persona review via context). Pre-fix run archived to `_submissions/_archive/run-20260425-pre-fix/`. Commits: `eb7f17d` (O1), `af049e0` (O2), `ece621e` (O3), `8269f20` (protocol doc fixes), `7d24b19` (spec GPT identifier), `595747b` (submissions archive + re-run). | O1+O2 root causes confirmed empirically: GPT and Nemotron now return full reviews with attachment access. O3 headers are cosmetic uniformization. Sonar remains attachment-unsupported; Perplexity routes `pplx_alpha` to its own deep-research system that ignores S3 attachments. Next: GitHub Issues for 6 REST-path reviews (Gemini direct by Robinson manually). | Commits `eb7f17d`, `af049e0`, `ece621e`; cross-check: `docs/api-research/three-capture-cross-check.md` | 2026-04-25 |
...
- **Orchestrator fix O1+O2+O3 2026-04-25 ✅** — GPT+Nemotron FIXED (full reviews), re-run `run-20260425T205535Z`, commits `eb7f17d`+`af049e0`+`ece621e`
- Tracker D60 formalizován

**Další krok (priorita):**
1. **GitHub Issues** — 6 REST recenzí z `run-20260425T205535Z` (Gemini direct = Robinson manuálně)
