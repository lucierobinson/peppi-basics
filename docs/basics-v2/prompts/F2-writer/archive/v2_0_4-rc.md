...
interface RecipeStep {
  ...
  // "prep"     — tvarování polotovaru / chlazení / odpočinek / odvážení bez nové gramáže
  //              (v2.0.4, H1 exempt, viz H1). Polotovar už je vytvořen v předchozím kroku,
  //              tento krok s ním pracuje (vyválení, odpočinek v lednici, tvarování).
...
Každý záznam v `ResearchBrief.failure_tracking.common_failures[]` MUSÍ mít non-null `when_detectable` field s validní hodnotou (`"during_process"` | `"immediately_after"` | `"after_cooling"` | `"next_day"`). Pokud kterýkoli FailureMode chybí `when_detectable` nebo má nevaliditní hodnotu → gate fail. Writer nemůže routovat 3C vs what_if_box podle H10. (HARD protějšek: H10 ověřuje, že routing byl proveden správně.)
...
- `step_type: "prep"` (v2.0.4) — tvarování polotovaru / chlazení / odpočinek / odvážení BEZ vstupní nové gramáže (např. `**Těsto craquelinu** — vyvácej mezi dvěma listy pečicího papíru na **2 mm**.` nebo `**Kuličky těsta** — odpočiň 30 min v lednici.`). Polotovar už existuje z předchozího kroku; prep s ním pracuje. **Prep vs. standard**: pokud krok začíná NOVOU gramáží/ingrediencí, je to `standard`. Pokud bere existující polotovar a tvaruje/chladí/odpočívá ho, je to `prep`. **Prep vs. assembly**: prep pracuje s JEDNÍM polotovarem (tvarování). Assembly kombinuje DVA a víc polotovarů dohromady.
...
| 14 (v2.0.3, v2.0.4) | `step_type: "preheat"`, `"tasting"`, `"assembly"` nebo `"prep"` bez explicit hodnoty | Exempt kroky MUSÍ mít explicit `step_type`, jinak H1 vyžaduje gramáž |
...
3. **Gramáže v 3A (HARD protějšek G1)** — Každý krok s `step_type: "standard"` (nebo bez step_type) má `instruction` začínající `**gramáž**`? Exempt typy (`preheat`, `tasting`, `assembly`, `prep`) se z této kontroly vylučují — ale MUSÍ mít explicitní `step_type` hodnotu. Pokud G1 prošel, ale zde ISSUE → Writer nezapsal gramáže, které F1 dodal (Writer bug, ne F1 bug).
4. **2A ↔ 3A (HARD protějšek G2)** — Všechny ingredience z 2A se objeví v 3A a naopak? Pokud ISSUE, uveď v `description` konkrétní missing/extra ingredience jmenovitě (např. `"mléko v 2A, chybí v 3A"`). Pro composite products: kontrola per component.
